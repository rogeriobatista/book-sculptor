...
class Chapter:
    ...
    def has_heading(self) -> bool:
        ...
        if self.full_label.strip():
            return True
        if self.kind in {"prologue", "epilogue"}:
            return True
    ...
    def display_label(self) -> str:
        ...
        if self.kind == "prologue":
            return f"Prólogo — {self.title}" if self.title else "Prólogo"
        if self.kind == "epilogue":
            return f"Epílogo — {self.title}" if self.title else "Epílogo"
        if not self.has_heading:
            return self.title.strip() or "Texto"
        if self.number is not None and self.title.strip():
            return f"Capítulo {self.number} — {self.title}"
